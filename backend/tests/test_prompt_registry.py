...
def test_get_prompt_returns_template_and_caches(monkeypatch):
    ...
    class _Row:
        template = "Investigate: {equipment_name}"

    monkeypatch.setattr(registry, "SessionLocal", lambda: _FakeSession([_Row()]))

    result1 = registry.get_prompt("investigate_anomaly")
    assert result1 == "Investigate: {equipment_name}"

    # Second call must hit the cache, not the DB -- the fake queue is now
    # empty, so a DB call here would raise IndexError.
    result2 = registry.get_prompt("investigate_anomaly")
    assert result2 == "Investigate: {equipment_name}"
...
def test_render_prompt_formats_kwargs(monkeypatch):
    monkeypatch.setattr(registry, "get_prompt", lambda name: "Text: {equipment_name}")

    rendered = registry.render_prompt("investigate_anomaly", equipment_name="Compressor C-201")

    assert rendered == "Text: Compressor C-201"
...
def test_add_prompt_version_increments_and_activates(monkeypatch):
    ...
    new_version = registry.add_prompt_version("investigate_anomaly", "new template text", activate=True)
